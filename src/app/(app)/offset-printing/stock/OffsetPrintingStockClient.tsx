...
interface OffsetPrintingStockClientProps {
  rolls: Roll[];
  tab: string;
}

export function OffsetPrintingStockClient({ rolls, tab }: OffsetPrintingStockClientProps) {
  ...
  const stockRows = useMemo(() => {
    ...
    return Array.from(groupsMap.values()).sort((a, b) => a.roll_id.localeCompare(b.roll_id));
  }, [rolls, activeTab]);
  ...
  return (
    <div className={cn("space-y-6 transition-opacity", isPending && "opacity-60")}>
      ...
      <div className="flex items-center gap-2 mb-5 no-print">
        <button
          onClick={() => {
            ...
            startTransition(() => {
              router.push("/offset-printing/stock?tab=available" as any);
            });
          }}
          className={cn(
        ...
        <button
          onClick={() => {
            setActiveTab("all");
            startTransition(() => {
              router.push("/offset-printing/stock?tab=all" as any);
            });
          }}
          className={cn(
            "px-4 py-2.5 rounded-lg text-sm font-bold transition-all duration-200 flex items-center gap-1.5",
      ...
      <Card>
        <CardHeader>
          <CardTitle>{activeTab === "all" ? "All Offset Printing Stock" : "Available Offset Printing Stock"} Summary</CardTitle>
        </CardHeader>
        <CardContent>
          {stockRows.length === 0 ? (
