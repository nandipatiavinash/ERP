...
export default async function RawMaterialsPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  const [{ data }, { data: purchases }] = await Promise.all([
    supabase.from("raw_materials").select("id, material_name, unit, opening_stock, current_stock, status").is("deleted_at", null).order("material_name", { ascending: true }).limit(500),
    supabase
