...
export function StockLaminationRollsClient({ rolls, fabricName }: StockLaminationRollsClientProps) {
  ...
  const totalMeters = useMemo(() => rolls.reduce((sum, r) => sum + Number(r.meters), 0), [rolls]);
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
        ...
        <div className="text-xs font-semibold text-slate-500 font-mono">
          Total Weight: {formatNumber(totalWeight, 1)} kg · Total Meters: {formatNumber(totalMeters, 0)} m
        </div>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>Available Laminated Rolls ({filteredRolls.length})</CardTitle>
        </CardHeader>
        <CardContent>
          {sortedRolls.length === 0 ? (
            <EmptyState title="No rolls found" description="No laminated rolls matching your criteria." />
          ) : (
