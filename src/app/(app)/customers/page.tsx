...
export default async function CustomersPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  const { data } = await supabase.from("customers").select("id, customer_name, phone, gst_number, address, status").is("deleted_at", null).order("created_at", { ascending: false }).limit(500);
