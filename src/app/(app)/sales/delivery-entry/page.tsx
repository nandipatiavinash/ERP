...
export default async function DeliveryEntryPage({
  ...
  const availablePromises = tab === "pending"
    ? [
        ...
        supabase.from("finishing_bundles").select("id, bundle_id, weight_kg, num_bags, status, fabric_type_id, product_id, finish_type, offset_rolls:source_offset_roll_id(offset_type), lamination_rolls:source_lam_roll_id(lam_type)").eq("status", "available").is("deleted_at", null),
  ...
  const [
    ...
    uniqueRollIds.length > 0 ? supabase.from("lamination_rolls").select("id, roll_id, s_no, supplier_roll_id, weight_kg, gross_weight, core_weight, net_weight, meters, status, fabric_type_id, product_id, lam_type, film_roll_id, roto_metallic_rolls(source_film_roll_id, roto_film_rolls(brand_id, film_type))").in("id", uniqueRollIds).is("deleted_at", null) : Promise.resolve({ data: [] }),
    ...
    uniqueRollIds.length > 0 ? supabase.from("finishing_bundles").select("id, bundle_id, weight_kg, num_bags, status, fabric_type_id, product_id, finish_type, offset_rolls:source_offset_roll_id(offset_type), lamination_rolls:source_lam_roll_id(lam_type)").in("id", uniqueRollIds).is("deleted_at", null) : Promise.resolve({ data: [] }),
